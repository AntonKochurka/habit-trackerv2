...
def _create_token(data: dict, expires_delta: timedelta) -> tuple[str, int]:
    ...
    to_encode.update({"exp": expire_at, "jti": str(uuid4())})
...
def verify_token(token: str, credentials_exception: Exception | None = None):
    try:
        payload = jwt.decode(token, config.SECRET_KEY, algorithms=[config.ALGORITHM])
        id = payload.get("sub")
        if id is None and credentials_exception is not None:
